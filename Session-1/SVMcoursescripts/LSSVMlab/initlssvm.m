function model = initlssvm(X,Y,type, gam,sig2, kernel_type, global_opt, preprocess)
    ...
    if type(1)~='c'
        if type(1)~='o'
            if type(1)~='s'
                if type(1)~='t' 
                    if type(1)~='N'
                        error('type has to be ''function (estimation)'', ''classification'', ''oneclass'', ''snd'',''timeserie'' or ''NARX''');
                    end
                end
            end
        end
...
%
% initializing global optimization
%
try model.global_opt = global_opt; catch, model.global_opt = 'csa'; end
