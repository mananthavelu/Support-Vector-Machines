...
%title(['Kernel PCA - Denoised datapoints in red',num2str(nc),' Approx:',num2str(approx)])
disp('Press any key to continue');
pause;
...
xdl=dat*U_lin(:,2)*U_lin(:,2)';
plot(xdl(:,1),xdl(:,2),'r+');

title('Linear PCA - Denoised data points using the first principal component');
%title(['Linear PCA - Denoised data points using the first principal component',num2str(nc),' Approx:',num2str(approx)])
