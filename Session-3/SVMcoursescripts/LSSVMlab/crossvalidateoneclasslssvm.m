function cost = crossvalidateoneclasslssvm(model,Y, L, omega, estfct,combinefct)
...
% remove/isolate otliers from the model
if sum(Y(:,1)~=1) > 0,
    omega_out = omega(Y(:,1)~=1,Y(:,1)==1);
    Y_out = Y(Y(:,1)~=1,:);
end

omega = omega(Y(:,1)==1,Y(:,1)==1);
Y = Y(Y(:,1)==1,:);
...
for j =1:numel(gams)
    ...
    % calculate matrix for OneClass LS-SVM once for the entire data
    S = ones(nb_data,1);
    Inb = eye(nb_data);
    ...
    try R = chol(Atot);
        ...
        Ri = R\Inb;
        C = Ri*Ri' - bias*(q)*q';
    catch ex%R = cholinc(sparse(Atot),1e-5);
        A = [(K+Inb./model.gam) -S; -S' 0];
        C = pinv(A);
        alpha = C*[zeros(nb_data,1);1];
    ...
    for l = 1:L,
        ...
        [~,yh] = postlssvm(model,[],betak);
        costs(l,j) = feval(estfct,Y(validation,:),sign(yh));
    end
    
    if exist('Y_out', 'var'),
        %yh = kernel_matrix2(omega,model.kernel_type,model.kernel_pars,d)*alpha-bias;
        yh_out = -kernel_matrix2(omega_out,model.kernel_type,model.kernel_pars,d)*alpha+bias;
        costs(:,j) = costs(:,j) + feval(estfct,Y_out,sign(yh_out)) / L; 
    end
end
