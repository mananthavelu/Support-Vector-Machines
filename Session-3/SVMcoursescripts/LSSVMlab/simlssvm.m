...
%
Yl = Y;
if (model.type(1)=='c' || model.type(1)=='o') && strcmp(model.latent,'no'),
    Y = 2*(Y>0)-1;
end
...
% postprocessing...
if model.preprocess(1)=='p' && ~((model.type(1)=='c' || model.type(1)=='s') && strcmp(model.latent,'yes')),
...
% decode if supervised novelty detection
if model.type(1)=='s' && ~strcmpi(model.codetype,'none' ) && ~strcmpi(model.code,'original'),
    [v, Yt] = max(Yt(:,1:model.y_dim-1),[],2);
    Yt(v < 0) = model.y_dim;
end
